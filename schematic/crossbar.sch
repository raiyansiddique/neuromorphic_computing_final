v {xschem version=3.4.3 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
N 140 -540 140 -10 {
lab=Bmeas}
N 260 -540 260 -10 {
lab=Ameas}
N 390 -530 390 -20 {
lab=#net1}
N 390 -540 390 -530 {
lab=#net1}
N 100 -420 100 -410 {
lab=Bmeas}
N 200 -420 200 -410 {
lab=Ameas}
N 340 -410 390 -410 {
lab=#net1}
N 340 -420 340 -410 {
lab=#net1}
N 340 -200 390 -200 {
lab=#net1}
N 340 -220 340 -200 {
lab=#net1}
N 200 -220 200 -200 {
lab=Ameas}
N 100 -220 100 -200 {
lab=Bmeas}
N 100 -40 100 -20 {
lab=Bmeas}
N 200 -40 200 -20 {
lab=Ameas}
N 390 -20 390 -10 {
lab=#net1}
N 340 -20 390 -20 {
lab=#net1}
N 340 -40 340 -20 {
lab=#net1}
N 90 -460 90 -260 {
lab=VDD}
N 90 -260 90 -80 {
lab=VDD}
N 190 -260 190 -80 {
lab=VDD}
N 190 -460 190 -260 {
lab=VDD}
N 330 -260 330 -80 {
lab=VDD}
N 330 -460 330 -260 {
lab=VDD}
N 110 -460 110 -260 {
lab=Vss}
N 110 -260 110 -80 {
lab=Vss}
N 210 -260 210 -80 {
lab=Vss}
N 210 -460 210 -260 {
lab=Vss}
N 350 -260 350 -80 {
lab=Vss}
N 350 -460 350 -260 {
lab=Vss}
N 260 -10 260 0 {
lab=Ameas}
N 100 -410 120 -410 {
lab=Bmeas}
N 130 -410 140 -410 {
lab=Bmeas}
N 100 -200 120 -200 {
lab=Bmeas}
N 130 -200 140 -200 {
lab=Bmeas}
N 100 -20 110 -20 {
lab=Bmeas}
N 120 -20 140 -20 {
lab=Bmeas}
N 200 -410 230 -410 {
lab=Ameas}
N 240 -410 260 -410 {
lab=Ameas}
N 240 -200 260 -200 {
lab=Ameas}
N 200 -20 220 -20 {
lab=Ameas}
N 230 -20 260 -20 {
lab=Ameas}
N 200 -200 240 -200 {
lab=Ameas}
N 120 -410 130 -410 {
lab=Bmeas}
N 230 -410 240 -410 {
lab=Ameas}
N 220 -20 230 -20 {
lab=Ameas}
N 110 -20 120 -20 {
lab=Bmeas}
N 120 -200 130 -200 {
lab=Bmeas}
N 200 -120 340 -120 {
lab=#net2}
N 100 -120 200 -120 {
lab=#net2}
N 200 -300 340 -300 {
lab=#net3}
N 100 -300 200 -300 {
lab=#net3}
N 200 -500 340 -500 {
lab=#net4}
N 100 -500 200 -500 {
lab=#net4}
N 140 -10 140 -0 {
lab=Bmeas}
N 390 -10 390 -0 {
lab=#net1}
N 310 -610 440 -610 {
lab=S}
N 190 -610 310 -610 {
lab=S}
N -40 -610 -40 -550 {
lab=S}
N -40 -610 190 -610 {
lab=S}
N -40 -550 -40 -350 {
lab=S}
N -40 -350 -40 -170 {
lab=S}
N 90 80 210 80 {
lab=S_bar}
N 210 80 340 80 {
lab=S_bar}
N 340 80 530 80 {
lab=S_bar}
N 530 80 540 80 {
lab=S_bar}
N 540 -70 540 80 {
lab=S_bar}
N 540 -250 540 -70 {
lab=S_bar}
N 540 -450 540 -250 {
lab=S_bar}
N 70 80 90 80 {
lab=S_bar}
N -40 80 -10 80 {
lab=S}
N -40 -170 -40 80 {
lab=S}
N 630 -500 650 -500 {
lab=#net5}
N 630 -300 650 -300 {
lab=#net6}
N 630 -120 650 -120 {
lab=#net7}
N 40 -500 50 -500 {
lab=#net8}
N 40 -300 50 -300 {
lab=#net9}
N 40 -120 50 -120 {
lab=#net10}
N 60 -500 100 -500 {
lab=#net4}
N 60 -300 100 -300 {
lab=#net3}
N 60 -120 100 -120 {
lab=#net2}
N 540 -550 540 -450 {
lab=S_bar}
N 340 -500 450 -500 {
lab=#net4}
N 340 -300 450 -300 {
lab=#net3}
N 340 -120 450 -120 {
lab=#net2}
N 620 -300 630 -300 {
lab=#net6}
N 620 -120 630 -120 {
lab=#net7}
N 620 -500 630 -500 {
lab=#net5}
N 660 -120 670 -120 {
lab=#net11}
N 660 -300 670 -300 {
lab=#net12}
N 370 -500 370 -490 {
lab=#net4}
N 350 -460 370 -460 {
lab=Vss}
N 220 -500 220 -490 {
lab=#net4}
N 210 -460 220 -460 {
lab=Vss}
N 120 -500 120 -490 {
lab=#net4}
N 110 -460 120 -460 {
lab=Vss}
N 110 -260 120 -260 {
lab=Vss}
N 120 -300 120 -290 {
lab=#net3}
N 150 -460 150 -260 {
lab=W1}
N 150 -260 150 -80 {
lab=W1}
N 110 -80 120 -80 {
lab=Vss}
N 120 -120 120 -110 {
lab=#net2}
N 250 -460 250 -260 {
lab=W1}
N 210 -80 220 -80 {
lab=Vss}
N 250 -260 250 -80 {
lab=W1}
N 210 -260 220 -260 {
lab=Vss}
N 220 -300 220 -290 {
lab=#net3}
N 220 -120 220 -110 {
lab=#net2}
N 400 -460 400 -260 {
lab=W1}
N 400 -260 400 -80 {
lab=W1}
N 370 -120 370 -110 {
lab=#net2}
N 350 -80 370 -80 {
lab=Vss}
N 350 -260 370 -260 {
lab=Vss}
N 370 -300 370 -290 {
lab=#net3}
N 250 -370 400 -370 {
lab=W1}
N 150 -370 250 -370 {
lab=W1}
C {madvlsi/vdd.sym} 90 -80 3 0 {name=l1 lab=VDD}
C {madvlsi/vdd.sym} 190 -80 3 0 {name=l2 lab=VDD}
C {madvlsi/vdd.sym} 330 -80 2 0 {name=l3 lab=VDD}
C {madvlsi/vsource.sym} 700 -760 0 0 {name=Vdd
value=1.8}
C {madvlsi/gnd.sym} 700 -730 0 0 {name=l6 lab=GND}
C {madvlsi/vdd.sym} 700 -790 0 0 {name=l7 lab=VDD}
C {madvlsi/vsource.sym} 630 -760 0 0 {name=Vdd1
value=-1.8}
C {madvlsi/gnd.sym} 630 -730 0 0 {name=l4 lab=GND}
C {devices/lab_pin.sym} 630 -790 1 0 {name=p2 sig_type=std_logic lab=Vss}
C {devices/lab_pin.sym} 110 -80 3 0 {name=p1 sig_type=std_logic lab=Vss}
C {devices/lab_pin.sym} 210 -80 3 0 {name=p3 sig_type=std_logic lab=Vss}
C {devices/lab_pin.sym} 350 -80 3 0 {name=p4 sig_type=std_logic lab=Vss}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 100 -40 3 0 {name=X11}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 100 -220 3 0 {name=X1}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 100 -420 3 0 {name=X2}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 200 -420 3 0 {name=X3}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 340 -420 3 0 {name=X4}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 340 -220 3 0 {name=X5}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 200 -220 3 0 {name=X6}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 200 -40 3 0 {name=X7}
C {/home/madvlsi/neuromorphic_computing_final/schematic/memristor_emulator.sym} 340 -40 3 0 {name=X8}
C {madvlsi/tt_models.sym} 440 -940 0 0 {
name=TT_MODELS
only_toplevel=false
value=".option wnflag=1
.param MC_SWITCH=0.0
.lib ~/skywater/skywater-pdk/libraries/sky130_fd_pr_ngspice/latest/models/sky130.lib.spice tt"
}
C {devices/code.sym} 600 -950 0 0 {name=SPICE only_toplevel=false value=".param W=1
.param L=0.15
.tran 1n 50u
.save all"}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} -90 -500 0 0 {name=X9}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} -90 -300 0 0 {name=X10}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} -90 -120 0 0 {name=X12}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 140 -660 1 0 {name=X13}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 260 -660 1 0 {name=X14}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 390 -660 1 0 {name=X15}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 490 -500 0 0 {name=X16}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 490 -300 0 0 {name=X17}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 490 -120 0 0 {name=X18}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 390 130 3 0 {name=X19}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 260 130 3 0 {name=X20}
C {/home/madvlsi/neuromorphic_computing_final/schematic/tg.sym} 140 130 3 0 {name=X21}
C {/home/madvlsi/neuromorphic_computing_final/schematic/inverter.sym} -40 80 0 0 {name=X22}
C {madvlsi/gnd.sym} 20 110 0 0 {name=l5 lab=GND}
C {madvlsi/vdd.sym} 20 50 0 0 {name=l8 lab=VDD}
C {madvlsi/gnd.sym} 210 -820 0 0 {name=l9 lab=GND}
C {devices/lab_pin.sym} 210 -880 1 0 {name=p6 sig_type=std_logic lab=A}
C {madvlsi/vsource.sym} 210 -850 0 0 {name=VPre1
value=1.8}
C {devices/lab_pin.sym} 140 -700 1 0 {name=p5 sig_type=std_logic lab=A}
C {madvlsi/isource.sym} -160 -500 3 0 {name=I2
value="PULSE(40u 0 0s 100ns 100ns 10us 20us)"}
C {madvlsi/gnd.sym} -190 -500 1 0 {name=l11 lab=GND}
C {madvlsi/isource.sym} -160 -300 3 0 {name=I1
value="PULSE(10u 0 0s 100ns 100ns 10us 20us)"}
C {madvlsi/isource.sym} -160 -120 3 0 {name=I3
value="PULSE(1u 0 0s 100ns 100ns 10us 20us)"}
C {madvlsi/gnd.sym} -190 -300 1 0 {name=l10 lab=GND}
C {madvlsi/gnd.sym} -190 -120 1 0 {name=l12 lab=GND}
C {madvlsi/ammeter1.sym} 650 -300 3 0 {name=Vmeas1}
C {madvlsi/ammeter1.sym} 650 -120 3 0 {name=Vmeas2}
C {madvlsi/gnd.sym} -80 230 0 0 {name=l16 lab=GND}
C {devices/lab_pin.sym} -80 170 1 0 {name=p9 sig_type=std_logic lab=B}
C {devices/lab_pin.sym} 140 170 3 0 {name=p10 sig_type=std_logic lab=B}
C {devices/lab_pin.sym} 260 170 3 0 {name=p13 sig_type=std_logic lab=B}
C {devices/lab_pin.sym} 390 170 3 0 {name=p14 sig_type=std_logic lab=B}
C {madvlsi/gnd.sym} -560 -200 0 0 {name=l17 lab=GND}
C {madvlsi/vsource.sym} -560 -230 0 0 {name=VPre3
value="PULSE(1.8 0 0s 100ns 100ns 10us 20us)"}
C {devices/lab_pin.sym} -560 -260 1 0 {name=p15 sig_type=std_logic lab=S}
C {devices/lab_pin.sym} -40 -610 1 0 {name=p16 sig_type=std_logic lab=S}
C {madvlsi/ammeter1.sym} 50 -500 3 0 {name=Vmeas3}
C {madvlsi/ammeter1.sym} 50 -300 3 0 {name=Vmeas4}
C {madvlsi/ammeter1.sym} 50 -120 3 0 {name=Vmeas5}
C {devices/lab_pin.sym} 140 -520 2 0 {name=p12 sig_type=std_logic lab=Bmeas}
C {devices/lab_pin.sym} 260 -10 2 0 {name=p17 sig_type=std_logic lab=Ameas}
C {devices/lab_pin.sym} 80 80 3 0 {name=p18 sig_type=std_logic lab=S_bar}
C {/home/madvlsi/neuromorphic_computing_final/schematic/besrour_neuron_cm.sym} 730 -120 0 0 {name=X23}
C {/home/madvlsi/neuromorphic_computing_final/schematic/besrour_neuron_cm.sym} 730 -300 0 0 {name=X24}
C {/home/madvlsi/neuromorphic_computing_final/schematic/besrour_neuron_cm.sym} 720 -500 0 0 {name=X25}
C {madvlsi/ammeter1.sym} 650 -500 3 0 {name=Vmeas6}
C {madvlsi/vdd.sym} 720 -550 0 0 {name=l13 lab=VDD}
C {madvlsi/vdd.sym} 730 -350 0 0 {name=l14 lab=VDD}
C {madvlsi/vdd.sym} 730 -170 0 0 {name=l15 lab=VDD}
C {madvlsi/gnd.sym} 730 -70 0 0 {name=l19 lab=GND}
C {madvlsi/gnd.sym} 730 -250 0 0 {name=l20 lab=GND}
C {madvlsi/gnd.sym} 720 -450 0 0 {name=l21 lab=GND}
C {devices/lab_pin.sym} 780 -500 2 0 {name=p19 sig_type=std_logic lab=out1}
C {devices/lab_pin.sym} 790 -300 2 0 {name=p20 sig_type=std_logic lab=out2}
C {devices/lab_pin.sym} 790 -120 2 0 {name=p21 sig_type=std_logic lab=out3}
C {madvlsi/nmos4.sym} 370 -460 2 0 {name=M5
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/nmos4.sym} 220 -460 2 0 {name=M1
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/gnd.sym} 220 -430 0 0 {name=l22 lab=GND}
C {madvlsi/gnd.sym} 370 -430 0 0 {name=l23 lab=GND}
C {madvlsi/nmos4.sym} 120 -460 2 0 {name=M2
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/gnd.sym} 120 -430 0 0 {name=l24 lab=GND}
C {madvlsi/nmos4.sym} 120 -260 2 0 {name=M3
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/nmos4.sym} 220 -260 2 0 {name=M4
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/nmos4.sym} 370 -260 2 0 {name=M6
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/nmos4.sym} 370 -80 2 0 {name=M7
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/nmos4.sym} 220 -80 2 0 {name=M8
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/nmos4.sym} 120 -80 2 0 {name=M9
L=0.15
W=4.5
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=nfet_01v8
spiceprefix=X
}
C {madvlsi/gnd.sym} 370 -230 0 0 {name=l25 lab=GND}
C {madvlsi/gnd.sym} 220 -230 0 0 {name=l26 lab=GND}
C {madvlsi/gnd.sym} 120 -230 0 0 {name=l27 lab=GND}
C {madvlsi/gnd.sym} 120 -50 0 0 {name=l28 lab=GND}
C {madvlsi/gnd.sym} 220 -50 0 0 {name=l29 lab=GND}
C {madvlsi/gnd.sym} 370 -50 0 0 {name=l30 lab=GND}
C {madvlsi/vsource.sym} 350 -860 0 0 {name=VPre5
value=0.8}
C {devices/lab_pin.sym} 350 -890 1 0 {name=p22 sig_type=std_logic lab=W1}
C {madvlsi/gnd.sym} 350 -830 0 0 {name=l31 lab=GND}
C {devices/lab_pin.sym} 400 -370 2 0 {name=p23 sig_type=std_logic lab=W1}
C {madvlsi/vsource.sym} -80 200 0 0 {name=VPre6
value="PULSE(0 0.4 0s 1ns 1ns 100ns 200ns)"}
C {devices/lab_pin.sym} 260 -700 1 0 {name=p7 sig_type=std_logic lab=A}
C {devices/lab_pin.sym} 390 -700 1 0 {name=p8 sig_type=std_logic lab=A}
